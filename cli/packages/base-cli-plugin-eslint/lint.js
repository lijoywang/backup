...
module.exports = function lint(args = {}, api) {
  ...
  const dotFiles = ['.*.js', '{src,tests}/**/.*.js'].filter(
    (pattern) => globby.sync(path.join(cwd, pattern)).length
  );
  ...
  const defaultFilesToLint = [
    ...
    .filter((pattern) =>
      globby.sync(path.join(cwd, pattern)).some((p) => !engine.isPathIgnored(p))
    )
    .concat(dotFiles);
  ...
  const maxWarnings =
    typeof argsConfig.maxWarnings === 'number'
      ? argsConfig.maxWarnings
      : Infinity;
  ...
  if (!isErrorsExceeded && !isWarningsExceeded) {
    ...
    if (isWarningsExceeded) {
      log(
        `Eslint found too many warnings (maximum: ${argsConfig.maxWarnings}).`
      );
    }
    process.exit(1);
  }
